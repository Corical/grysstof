...
async function discord<T>(path: string): Promise<T | null> {
  for (;;) {
    const res = await fetch(`${API}${path}`, { headers: { authorization: `Bot ${token}` }, signal: AbortSignal.timeout(15_000) });
    if (res.status === 429) {
...
const counts = { discord: all.length, needChannel: 0, needReply: 0, linked: 0, replyUnreadable: 0, notAReply: 0, written: 0, failed: 0, skippedMismatch: 0 };
